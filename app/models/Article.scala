...
object Article {
  ...
  var articles = Set(
    Article(
      ...
      "<p>Общая документация по языку. Краткий разбор семантики. Разбор классов, основные тонкости.</p>",
      "Общая документация по языку. Краткий разбор семантики. Разбор классов, основные тонкости.",
      1
    ),
    Article(
      1,
    ...
    Article(
      ...
      None,
      "SBT",
      "<p>Сборщик проектов для scala и java.</p>",
      "Сборщик проектов для scala и java.",
      2
    )
  )
  ...
  def toJsonTest(konspectId:Int) = {

    Json.toJson(
      articles.filter(_.konspectId == konspectId).toSeq.map(
        art =>
          graphElement(
            graphData(
      ...
      ) ++ articles.filter(_.konspectId == konspectId).toSeq.filter(_.parentId.isDefined).map(
        art =>
          graphElement(
            graphData(
              None,
